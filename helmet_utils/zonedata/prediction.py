...
def read_and_process_data(pop_file, wrk_file):
    ...
    df = pd.merge(pop_df[['pop_total', 'pop_0-6', 'pop_7-17', 'pop_18-29', 'pop_30-49', 'pop_50-64', 'pop_65-']],
                 wrk_df[['wrk_total', 'wrk_serv', 'wrk_shop', 'wrk_logi', 'wrk_indu', 'wrk_other']],
                 left_index=True, right_index=True)
    df = df[['pop_total', 'wrk_total']]  # Keep only total population and total workplaces, possibly use grouped data in the future
...
def main():
    ...
    # Optional model selection, with hyperparameter tuning. Gradient boosting regression is already chosen, but left the code here for future reference.
    models = [
